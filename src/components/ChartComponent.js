...
import { AddBox, ArrowDownward, Check, ChevronLeft, ChevronRight, Clear, DeleteOutline, Edit, FirstPage, LastPage, ViewColumn, SaveAlt, FilterList, Search, Remove } from "@material-ui/icons";
...
export default function MyChart({maquina,machine, currentItemChart, selectedMaquina, kWh}) {
         ...
         return{
          ...
          maquina = machine.map(({ velocidade_fio}) => {
           return{
            ...
            console.log("Dados da máquina a serem passados para os eixos X e Y: ", machine);
   ...
   const data = React.useMemo(
    ...
    const axes = React.useMemo(
      () => {
        ...
        { primary: true, type: 'linear', position: 'bottom', format: d => `${tempo()}${d}` },
        { type: 'linear', position: 'left', stacked: false}
      ]
    else return[
      { primary: true, type: 'ordinal', position: 'bottom', format: d => `${tempo()}${d}` },
    ...
    return (
      ...
      <h5 style={{ color: "#333" , padding: "0.5rem", fontSize: "15px"}}>Gráfico:</h5> 
            <div className="toggle "> 
                <button className={"toggle barras" + (active === bar ? " active" : "")}>Barras</button>
                <input type="checkbox" name="toggleChartType" onChange={handleCheckbox}></input>
                <button className={"toggle linhas" + (active === line ? " active" : "")}>Linhas</button>
                </div>
                <p><BsIcons.BsTable size={20} color='#E4181D'/></p>
                
     <h5 style={{ color: "#333" , fontSize: "15px"}}>Tabela de Dados:</h5>
     
                <button onClick={toggleModalTabela} className="tabela">Visualizar <HiIcons.HiOutlineViewList size={20}/></button>
                <Modal 
                      size="lg"
                      aria-labelledby="contained-modal-title-vcenter"
                      centered
                      isOpen={isModalOpen} toggle={toggleModalTabela} modalTransition={{ timeout: 500 }} 
                >
                  <ModalHeader toggle={toggleModalTabela}> Listagem de Produção: </ModalHeader>                
        ...
        <Card style={{border: "none", background:"none", margin:"0", padding:"0"}} >
                               <h6 style={{padding: "1rem"}}>Quantidade Eléctrica (KW.h)</h6>
                               <h1 style={{ color: "#333" , paddingBottom: "1rem", fontSize: "55px"}}>{kWh}</h1>
                              
                           </Card>
                           <div className='detalhes'>
                             <button style={{fontSize: "9px"}}>Current(mA)<h4>{Math.round((kWh*10000)*1.20)}</h4></button>
                             <button style={{fontSize: "9px"}}>Power(W)<h4>{Math.round((kWh*10000)*1.10)}</h4></button>
                             <button style={{fontSize: "9px"}}>Voltage(V)<h4>{Math.round((kWh*1000)/1.30)}</h4></button>
                             <button style={{fontSize: "9px"}}>Total(kW.h)<h4>{Math.round(kWh*115)}</h4></button>
                           </div>
                           <div><h6 style={{fontSize: "12px", padding: "20px"}}>Mês passado: <strong>1.250 kW</strong></h6></div>
